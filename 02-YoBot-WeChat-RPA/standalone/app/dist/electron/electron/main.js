import { RecoveryMcp } from '../../../recovery-mcp.js';
let recoveryMcp;
...
ipcMain.handle('app:update-check', async () => process.env.YOBOT_RECOVERED === '1' ? null : desktopUpdateManager?.checkForUpdates(false) || null);
        ...
        child.on('message', async (msg) => {
            ...
            if (msg.type === 'rpa:mcp-control-request' && typeof msg.requestId === 'string') {
                ...
                if (process.platform !== 'darwin' || !macOSControlTransport
                    ...
                    if (!recoveryMcp) recoveryMcp = new RecoveryMcp(async request => {
                        const target = await macOSControlDriver.resolveOwnedLoopback();
                        if (request.path === '/api/contact/sync') {
                            const response = await axios.request({
                                url: `${target.apiBaseUrl}${request.path}`,
                                method: request.method,
                                headers: { 'Content-Type': 'application/json', 'X-API-Key': target.credential.read(), 'X-Channel-ID': target.ownership.channel_id },
                            ...
                            return { success: true, status: response.status, bodyText: typeof response.data === 'string' ? response.data : JSON.stringify(response.data) };
                        }
                        const response = await fetch(`${target.apiBaseUrl}${request.path}`, {
                            method: request.method,
                            headers: { 'Content-Type': 'application/json', 'X-API-Key': target.credential.read(), 'X-Channel-ID': target.ownership.channel_id },
                            ...(request.body ? { body: JSON.stringify(request.body) } : {}),
                            redirect: 'error', signal: AbortSignal.timeout(15000),
                        });
                        return { success: true, status: response.status, bodyText: await response.text() };
                    });
                    if (recoveryMcp.server) {
                        const health = body.enabled ? await macOSControlTransport.execute({ path: '/api/health', method: 'GET' }) : null;
                        if (health && (!health.success || health.status !== 200)) result = health;
                        else {
                            const payload = await recoveryMcp.configure(body);
                            result = { success: true, status: 200, contentType: 'application/json', bodyText: JSON.stringify(payload) };
                        }
                    } else {
...
function createWindow() {
    ...
    // Keep the recovered renderer bundle intact and apply the neutral, flat
    // visual language as an auditable presentation-only layer.
    mainWindow.webContents.on('did-finish-load', () => {
        try {
            const flatTheme = fs.readFileSync(path.join(__dirname, '../../../recovery-main-flat.css'), 'utf8');
            mainWindow?.webContents.insertCSS(flatTheme, { cssOrigin: 'user' })
                .catch(error => console.warn('[Main] failed to apply flat theme:', error));
        }
        catch (error) {
            console.warn('[Main] flat theme is unavailable:', error);
        }
    });
    ...
    replaceWindowIpcListener('rpa:open-config-window', async (event, url) => {
        ...
        if (rpaConfigWindow && !rpaConfigWindow.isDestroyed()) {
            ...
            rpaConfigWindow.webContents.on('did-finish-load', () => {
                ...
                const syncErrorAdapter = fs.readFileSync(path.join(__dirname, '../../../recovery-rpa-errors.js'), 'utf8');
                rpaConfigWindow?.webContents.executeJavaScript(syncErrorAdapter).catch(e => console.error('[Recovery] RPA error adapter failed:', e.message));
                const flatTheme = fs.readFileSync(path.join(__dirname, '../../../recovery-rpa-flat.css'), 'utf8');
                rpaConfigWindow?.webContents.insertCSS(flatTheme, { cssOrigin: 'user' }).catch(e => console.error('[Recovery] RPA theme failed:', e.message));
                const scrollbarCss = `
