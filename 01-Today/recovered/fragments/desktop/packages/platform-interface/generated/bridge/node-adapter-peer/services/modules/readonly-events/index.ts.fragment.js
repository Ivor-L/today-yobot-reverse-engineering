// Compiled fragment from ../../packages/platform-interface/generated/bridge/node-adapter-peer/services/modules/readonly-events/index.ts.
// The original TypeScript and import graph are not restored.


// Generated by scripts/generate.mts. Do not edit.


class ReadonlyEvents {
    async subscribe(eventName, listener) {
        const safeListener = async (event)=>{
            try {
                await listener(event);
            } catch  {
            // Observers must not fail bridge operations or block other observers.
            }
        };
        this.events.on(eventName, safeListener);
        return Object.freeze({
            unsubscribe: async ()=>{
                this.events.off(eventName, safeListener);
            }
        });
    }
    emit(eventName, event) {
        return this.events.emit(eventName, event);
    }
    constructor(){
        this.events = new node_modules_eventemitter3();
    }
}
ReadonlyEvents = __decorate([
    injectable()
], ReadonlyEvents);
